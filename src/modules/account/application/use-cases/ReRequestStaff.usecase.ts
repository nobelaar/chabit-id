...
export interface ReRequestStaffDto { callerRef: string; }

export class ReRequestStaffUseCase {
  constructor(private readonly repo: AccountRepository, private readonly eventRepo: AccountEventRepository) {}

  async execute(dto: ReRequestStaffDto): Promise<void> {
    const callerRef = IdentityRef.fromPrimitive(dto.callerRef);
    const account = await this.repo.findByIdentityRefAndType(callerRef, AccountType.staff());
    if (!account) throw new AccountNotFoundError();
    account.reRequest();
    await this.repo.save(account);
    this.eventRepo.save({ accountId: account.getId(), type: 're_requested', performedBy: callerRef })
      .catch(err => logger.warn({ err }, '[ReRequestStaff] event error'));
  }
}
