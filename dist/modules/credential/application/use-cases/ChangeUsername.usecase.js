...
import { CredentialNotFoundError, CannotChangeUsernameYetError, UsernameReservedError, UsernameAlreadyTakenError, } from '../../domain/errors/Credential.errors.js';
const COOLDOWN_MS = 30 * 24 * 60 * 60 * 1000; // 30 days
export class ChangeUsernameUseCase {
    repo;
    reservedList;
    constructor(repo, reservedList) {
        this.repo = repo;
    ...
    async execute(dto) {
        ...
        if (this.reservedList.isReserved(newUsername))
            throw new UsernameReservedError(dto.newUsername);
        const existing = await this.repo.findByUsername(newUsername);
        if (existing)
            throw new UsernameAlreadyTakenError(dto.newUsername);
