...
import { signInSchema, refreshTokenSchema, changePasswordSchema, changeUsernameSchema, resetPasswordSchema, forgotPasswordSchema, } from './credential.schemas.js';
...
export function createCredentialRoutes(signInUseCase, refreshTokenUseCase, revokeTokenUseCase, changePasswordUseCase, revokeAllTokensUseCase, changeUsernameUseCase, resetPasswordUseCase, requestVerificationUseCase) {
    ...
    // POST /auth/sign-in
    router.post('/sign-in', zValidator('json', signInSchema, (result, c) => {
        if (!result.success)
    ...
    // POST /auth/refresh
    router.post('/refresh', zValidator('json', refreshTokenSchema, (result, c) => {
        if (!result.success)
            return c.json({ error: 'VALIDATION_ERROR', message: 'Invalid request body', details: result.error.issues }, 400);
    }), async (c) => {
        const body = c.req.valid('json');
    ...
    router.post('/sign-out', async (c) => {
        ...
        if (!sessionId)
            return c.json({ error: 'MISSING_SESSION', message: 'x-session-id header required' }, 400);
        await revokeTokenUseCase.execute({ sessionId });
        return c.json({ message: 'Signed out successfully' }, 200);
    });
    // PATCH /auth/change-password
    router.patch('/change-password', zValidator('json', changePasswordSchema, (result, c) => {
        if (!result.success)
            return c.json({ error: 'VALIDATION_ERROR', message: 'Invalid request body', details: result.error.issues }, 400);
    }), async (c) => {
    ...
    router.patch('/change-username', zValidator('json', changeUsernameSchema, (result, c) => {
        if (!result.success)
            return c.json({ error: 'VALIDATION_ERROR', message: 'Invalid request body', details: result.error.issues }, 400);
    }), async (c) => {
        const identityRef = c.req.header('x-identity-ref');
    ...
    router.post('/reset-password', resetPasswordLimiter, zValidator('json', resetPasswordSchema, (result, c) => {
        if (!result.success)
            return c.json({ error: 'VALIDATION_ERROR', message: 'Invalid request body', details: result.error.issues }, 400);
    }), async (c) => {
        const body = c.req.valid('json');
    ...
    router.post('/forgot-password', forgotPasswordLimiter, zValidator('json', forgotPasswordSchema, (result, c) => {
        if (!result.success)
            return c.json({ error: 'VALIDATION_ERROR', message: 'Invalid request body', details: result.error.issues }, 400);
    }), async (c) => {
        const body = c.req.valid('json');
