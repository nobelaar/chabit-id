...
export function createVerificationRoutes(requestUseCase, verifyUseCase) {
    ...
    router.post('/email', requestLimiter, zValidator('json', requestVerificationSchema, (result, c) => {
        if (!result.success) {
    ...
    router.post('/email/verify', verifyLimiter, zValidator('json', verifyEmailSchema, (result, c) => {
        if (!result.success) {
            return c.json({ error: 'VALIDATION_ERROR', message: 'Invalid request body', details: result.error.issues }, 400);
        }
    }), async (c) => {
